...
export class SyncService extends BaseService {
        ...
        invoices.map(async (invoice) => {
          if (invoiceIds.includes(invoice.id)) {
            switch (eventType) {
              ...
              default:
                console.error(
                  'SyncLogService#processSync | Unknown event type: ',
                  eventType,
                )
                break
            }
  ...
  async syncFailedRecords() {
    ...
    const failedSyncLogs = await syncLogService.getFailedSyncLogsByEntityType()

    console.info('\nFetched failed logs', failedSyncLogs)
