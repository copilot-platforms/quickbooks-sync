...
export default class IntuitAPI {
  ...
  async _getAllItems(limit: number, columns: string[] = ['Id']) {
    ...
    const customerQuery = `select ${stringColumns} from Item where Type = 'Service' maxresults ${limit}` // Other items with type "Category" cannot be used in invoice line item. It throws an error.
    CustomLogger.info({
      obj: { customerQuery },
