...
export interface PayoutReconciliationTestHandle {
 ...
 * for payout.reconciliation_completed tests. Mirrors `setupPaymentSucceededTest`;
 * `optsFactory` runs once per test so override `vi.fn()`s are freshly instantiated.
 */
export function setupPayoutReconciliationTest(
  optsFactory?: () => InstallOpts,
): PayoutReconciliationTestHandle {
  const handle = {} as PayoutReconciliationTestHandle
